...
void draw_sh_samples(
    FuncParse<decltype(ProjectSparseSamples)>::ResultType const& sh_coeffs,
    slice1d<vec3> const& subsampled_rotations,
    const vec3 limit_position,
    const mat3 limit_rotation,
    vec3 const space_offset,
    float const scale) {

    for (int i = 0; i < subsampled_rotations.size; i++)
    {

        auto limit_space_rotation = subsampled_rotations(i);

        auto rotation_length = length(limit_space_rotation);
        if (rotation_length < 0.01f) {
            continue;
        }
       
        limit_space_rotation = normalize(limit_space_rotation);

        auto limit_vec = Eigen::Vector3d{ limit_space_rotation.x, limit_space_rotation.y, limit_space_rotation.z };
        double phi, theta;
        ToSphericalCoords(limit_vec, &phi, &theta);
        limit_vec = ToVector(phi, theta);
        double sh_length = EvalSHSum(4, *sh_coeffs.get(), limit_vec);
        
        vec3 limit_rotation_vec = sh_length * vec3{float( limit_vec.x()), float(limit_vec.y()), float(limit_vec.z()) };
        limit_rotation_vec = mat3_mul_vec3(limit_rotation, limit_rotation_vec) + limit_position;

    
        DrawSphereWires(
            to_Vector3(space_offset + scale * limit_rotation_vec),
            0.005f,
            4, 6,
            Color{ 255, 00, 0, 50 });
    }
...
void draw_sh_all_limit_samples(
    FuncParse<decltype(ProjectSparseSamples)>::ResultType const& sh_coeffs,
    slice1d<vec3> const& subsampled_rotations,
    const vec3 limit_position,
    const mat3 limit_rotation,
    ...
    int const limit_count = 18;

    float start_phi = 0;
   
    float delta_radian = sh::M_PI / limit_count;

    while (start_phi < sh::M_PI * 2.0) {
        float start_theta = 0;
        while (start_theta < sh::M_PI) {

            auto dir = ToVector(start_phi, start_theta);
            start_theta += delta_radian;

            double sh_length = EvalSHSum(4, *sh_coeffs.get(), dir);
            dir.normalize();

            vec3 limit_rotation_vec = sh_length * vec3{ float(dir.x()), float(dir.y()), float(dir.z()) };
            limit_rotation_vec = mat3_mul_vec3(limit_rotation, limit_rotation_vec) + limit_position;
            DrawSphereWires(
                to_Vector3(space_offset + scale * limit_rotation_vec),
                0.005f,
...
int main(void)
{
    ...
    std::vector<FuncParse<decltype(ProjectSparseSamples)>::ResultType> limit_sh_rotations;
    ...
    for (int j = 0; j < db.nbones(); ++j) {
        std::remove_const_t<std::remove_reference_t<FuncParse<decltype(ProjectSparseSamples)>::ArgParse::ArgType2>> dirs;  // reference不是const, 所以要先去除reference, 然后去除const ,才能得到提成type
        std::remove_const_t < std::remove_reference_t < FuncParse<decltype(ProjectSparseSamples)>::ArgParse::ArgType3>> rotation_values;
        
        auto limit_rotation = limit_rotations(j);
        auto limit_position = limit_positions(j);
        for (int i = 0; i < subsampled_limit_space_rotations[j].size; ++i) {
        
            auto limit_space_rotation = subsampled_limit_space_rotations[j](i);
           
            // Inverse transform point using position and rotation
            vec3 limit_point = mat3_transpose_mul_vec3(
               limit_rotation,
                limit_space_rotation - limit_position);
            //vec3 limit_point = limit_space_rotation;
            auto rotation_length = length(limit_point);
            if (rotation_length < 0.01f) {
                continue;
            }
            rotation_values.push_back(rotation_length);
            limit_point = normalize(limit_point);
            dirs.push_back({ limit_point.x, limit_point.y, limit_point.z });
        }
        // auto coeef_unique(ProjectSparseSamples(4, dirs, rotation_values));
        limit_sh_rotations.emplace_back(ProjectSparseSamples(4, dirs, rotation_values));
        // limit_sh_rotations.emplace_back(project(4, dirs, rotation_values))
    ...
    auto update_func = [&]()
    {
        ...
        else
        {
            ...
            draw_sh_samples(
                limit_sh_rotations[joint_index],
                subsampled_limit_space_rotations[joint_index],
                limit_positions(joint_index),
                limit_rotations(joint_index),
                space_offset,
